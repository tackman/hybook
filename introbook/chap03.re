...
著者の環境が Linux デスクトップなので、それを前提とした内容になっています。
...
当面は利用し続けることが期待できます。
また、Emacs の伝統的な Lisp プラグイン paredit なども利用することができます。

=== Vim

vim での Hy に関しては、現状はかなり厳しいと言わざるを得ません。
vim-hy プラグインはありますが、現在では unmentained な上に動作に問題が出ている状態です。

誰かが vim 向けプラグインを作れば、それがデファクトになるオープンソースチャンスという見方はあります。

=== VSCode

VSCode のマーケットにあるプラグインは、現状シンタックスハイライトがつくものしかないようです。
...
そのためドットファイル（.spacemacs）に一行追加をするだけで hy-mode と関連するプラグインが利用可能になります。
...
* vim, VSCodeに比べて相対的にはプラグインに恵まれている
* 導入が容易。Emacs や vim と異なり、プラグイン管理のためのプラグイン導入などが不要
...
また、既存の Emacs を乗っ取る形で動作します。通常の emacs との共存は困難なので、既存 Emacs ユーザーは注意が必要です。

Spacemacs の特色として、evil mode での vim 風キーバインドがあります。
「Emacs がいいんだけど vim のキーバインドが好きなんだよな〜」というハンバーグカレーになっています。
...
エディタを再起動するか、 SPC f e R　(dotspacemacs/sync-configuration-layers) コマンドを実行すると hy-layer に必要な
プラグインがインストールされます。これで hy-mode が利用可能になりました！

=== その他の設定

プラグインの導入は以上で完了するのですが、paredit など一部のプラグイン向けのキーバインドは
...
==== virtualenv だけではいけないのか

Docker での実行環境を作ることは強くおすすめしておきます。

 * ポータビリティを確保する一番確実な方法が Docker イメージの作成です。pipenv などのツールもありますが、信頼性に難があります
 * Dockerfile には環境構築、依存性を動くコードに残すことができます。
 * デプロイが必要なコードを書く場合、Docker イメージだけで済むようにするとインフラ作業が圧倒的に楽です
 * 複数人で環境を共有したい場合、ビルド済み Docker イメージを配布すれば手間と時間を節約できます

開発環境と本番環境の差を減らすという観点からは、開発時も Docker コンテナ上で実行するスタイルの方が望ましいでしょう。
それではやりきれない場合には、ホスト側の virtualenv などを併用することになります。
...
Lisp での開発と言えば REPL 駆動です（当社調べ）。Spacemacs での REPL 駆動についての導入です。
...
少し変わった例として、Cloud Functions へのデプロイを取り上げます。
