...
SomeClass().f('arg').g('fugafuga').h().f('Something result')
//}

chainマクロは、第1引数をオブジェクト、第2引数をメソッド名として取り、
メソッドにドット演算子でアクセスしています。
ここで第1引数には -> により前フォームの値が入るので、
「インスタンスを返すメソッドの返り値」が持つメソッドを取得できます。
取得したメソッドを呼び出すため、さらにカッコで囲みつつ引数を後ろに配置しているのが
chainマクロの中身です。